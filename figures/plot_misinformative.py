"""Section 5.3 Misinformative regime: data + CQR plots.

Generates:
  - MeanFailure_data.png            (scatter of LF/HF responses)
  - MeanFailure_cqr_HF-Only.png
  - MeanFailure_cqr_MFQR.png
  - MeanFailure_cqr_MFQR_plus_OS.png
  - MeanFailure_cqr_MFQR_plus_MS.png   (multi-step: m chosen by CV pinball)

The one-step correction is run in *raw HF response space* (one_step_space='raw'),
matching the §5.3 protocol from the paper: the bias is in the affine offset
...
def plot_data_figure(outdir):
    ...
    plt.tight_layout()
    out = os.path.join(outdir, "MeanFailure_data.png")
...
def main():
    ...
    ms_filename = f"MeanFailure_cqr_Iter_m{m_hat}.png"
    plot_plan = [
        ("MeanFailure_cqr_HF-Only.png",       lo_hf, up_hf, lo_hf_c, up_hf_c, "#d62728"),
        ("MeanFailure_cqr_MFQR.png",          lo_mq, up_mq, lo_mq_c, up_mq_c, "#2ca02c"),
        ("MeanFailure_cqr_MFQR_plus_OS.png",  lo_os, up_os, lo_os_c, up_os_c, "#1f77b4"),
    ...
    for name, cqr_lo, cqr_up, col, raw_lo, raw_up in finalized:
        ...
        label = name.replace("MeanFailure_cqr_", "").replace(".png", "")
